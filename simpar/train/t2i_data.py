...
class T2IDataset(Dataset):
    def __init__(
        self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, image_size=384, aug="resize", data_args=None
    ):
        ...
        if aug == "resize":
            ...
            augmentations = transforms.Compose(
                [transforms.ToTensor(), transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True)]
            )
...
class EvalT2IDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        sources = [[{"from": "human", "value": "<image>"}, {"from": "gpt", "value": prompt}]]
        ...
        uncond_t2i_data_dict = preprocess_t2i(sources2, self.tokenizer, vtokens_shape=0, p_drop_cond=0.0)
