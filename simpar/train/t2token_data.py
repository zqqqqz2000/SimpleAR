...
class T2TokenDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        token_path = (
            sources["code_path"] if self.data_dir is None else os.path.join(self.data_dir, sources["code_path"])
        )
        label_path = (
            sources["label_path"] if self.data_dir is None else os.path.join(self.data_dir, sources["label_path"])
        )
        data_type = "image"
        ...
        sources = [{"conversations": [{"from": "human", "value": "<image>"}, {"from": "gpt", "value": prompt}]}]

        sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), data_args=self.data_args)
        t2i_data_dict = preprocess_t2i(
            sources, self.tokenizer, vtokens_shape=vtokens_shape, p_drop_cond=self.p_drop_cond
        )
